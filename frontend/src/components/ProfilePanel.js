...
import { X, Flame, ChevronRight, LogOut, Trash2, Lock, Eye, EyeOff } from 'lucide-react';
...
export const ProfilePanel = ({ open, onClose }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setLinkedProfiles(Array.isArray(profilesRes.data) ? profilesRes.data : []);
    } catch {}
  }, [token]);
  ...
  const handleSwitchProfile = async (profileId) => {
    try {
      const res = await axios.post(`${BACKEND_URL}/api/auth/switch-profile`, { profile_id: profileId }, { headers });
